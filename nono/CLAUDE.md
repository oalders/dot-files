...
| `oalders-playwright`| `~/.cache/ms-playwright` (host Chromium bundle, **read-only**), `~/.cache/nono-playwright/ms-playwright` (in-sandbox e2e bundle, **r+w**), `/dev/shm` (browser IPC). `bin/nn` sets `PLAYWRIGHT_BROWSERS_PATH=$HOME/.cache/nono-playwright/ms-playwright` so every worktree (and the in-sandbox MCP) shares one bundle instead of re-downloading ~265 MB into each worktree's `XDG_CACHE_HOME` (#975). That dir is r+w so the first in-sandbox install seeds it; the system bundle stays read-only so an in-sandbox session can't poison a binary that a later host/un-sandboxed `playwright` run would execute outside Landlock. `bin/nn` pre-creates the writable dir (the grant can't, since its `~/.cache` parent stays denied). The read-only/writable split is enforced by Landlock, not by the env var: do **not** `export PLAYWRIGHT_BROWSERS_PATH` to the writable dir in host shell config (`bashrc`/`.envrc`), or un-sandboxed host `playwright` runs would execute binaries from it and the isolation collapses. Net-free; the browser-download CDN hosts live in the paired `oalders-playwright-net` (composed into `oalders.json`'s `extends`, not `oalders-core`'s, so only the default chain gets them). |
